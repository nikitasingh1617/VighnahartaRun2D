...
export function getCurrentButtons() {
  ...
  if (state.mode === 'instructions') {
    return [{ id:'back', label:'BACK TO MENU', x: cx-150, y: 480, w: 300, h: 44, accent:'#ffd24a' }];
  }
  ...
  if (state.mode === 'diffSelect') {
    ...
    return [{
      id:'name-go',
      label: ready ? '▶  LET\u2019S GO!' : 'TYPE A NAME TO BEGIN',
...
export function handleButton(id) {
  playClick();
  if (id === 'play') { initAudio(); state.mode = 'intro'; state.introT = 0; state.instructionsScrollY = 0; return; }
  if (id === 'play') { initAudio(); state.mode = 'intro'; state.introT = 0; return; }
  if (id === 'intro-continue') { initAudio(); state.mode = 'sceneSelect'; return; }
  ...
  if (id === 'restart') { initAudio(); state.delivered = 0; import('../gameplay/rounds.js').then(m => m.startRound(1)); return; }
  if (id === 'quit') { state.mode = 'menu'; return; }
  if (id === 'win-again') { initAudio(); state.delivered = 0; import('../gameplay/rounds.js').then(m => m.startRound(1)); return; }
  if (id === 'win-next') {
